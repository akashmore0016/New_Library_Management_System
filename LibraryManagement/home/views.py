...
def edit_book_view(request):
    books = TextBook.objects.get(isbn=request.GET['bookid'])
    context = {'book': books}
...
def update_book(request):
    ...
    books = TextBook.objects.get(isbn=request.POST['bookid'])
...
def add_members(request):
    ...
    members = Readers(members_id=members_id,
                      name=name,
                      email_id=email_id,
                      mobile_no=mobile_no)
...
def edit_members_view(request):
    members = Readers.objects.get(members_id=request.GET.get('members_id'))
    context = {'members': members}
...
def edit_members(request):
    ...
    members = Readers.objects.get(members_id=request.POST['members_id'])
...
def delete_members(request):
    members = Readers.objects.get(members_id=request.GET['members_id'])
    members.delete()
    return HttpResponseRedirect("/members")


def transactionView(request):
    transactions = Proceedings.objects.all()
    var = []
    for i in transactions:
        ...
        oneMember = Readers.objects.get(members_id=members_id)
        onebook = TextBook.objects.get(isbn=book_isbn)
        transData = ProceedingData(reciept_id, members_id, oneMember.name, book_isbn, onebook.title, issue_date,
...
def issue_book(request):
    ...
    try:
        ...
        if members_id == members_id and onemember.debt < 500:
            transaction = proceedings(members_id=members_id,
                                      book_isbn=book_isbn,
        ...
        context = {'error': "Member Id or TextBook ISBN does not match from the Database"}
        return render(request, 'debt.html', context)

def return_book_view(request):
    return_book = Proceedings.objects.get(transaction_id=request.GET.get('reciept_id'))
    ...
    print(transData)
    context = {'return_book': transData, 'form_title': 'Return TextBook'}
...
def return_book(request):
    ...
    onebook = TextBook.objects.get(isbn=book_isbn)
    onemember = Readers.objects.get(members_id=member_id)
    single_transaction = Proceedings.objects.get(transaction_id=receipt_id)
...
def search_book(request):
    ...
    try:
        book = TextBook.objects.filter(title__icontains=search_term, author__icontains=search_term1)
        context = {'book': book}
        return render(request, 'book.html', context)
    except TextBook.DoesNotExist:
        context = {"error_msg": "TextBook not found"}
...
def import_data(request):
    ...
    for single_book_dict in list_of_books_dict:
        ...
        single_library_book = LibraryBooks(**new_single_book_dict)
